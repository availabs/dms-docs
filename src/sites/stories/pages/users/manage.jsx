...
const  NewUserModal =  withAuth(({open, setOpen, apiLoad,apiUpdate,users }) => {
  const user = useAuth()
  ...
  const createUser = async () => {
    ...
    let resp = await apiUpdate({data: newUser, config: {format: MemberFormat}})
    ...
    <ModalContainer open={open} width={'sm:max-w-3xl '}>
      ...
      <Select
        ...
        onChange={(e) => {
          ...
          setNewuser({...newUser, user_id: e.target.value,email: user.email})
        }}
      >
        <option value={null} className='w-full text-center'> -- Select User -- </option>
        {authUsers
          .filter(d => userIds.indexOf(+d.id) === -1)
          .map(d => <option value={d.id} key={d.id}>{d.email} ({d.id})</option>)}
      </Select>
      ...
      >
        <option value={'user'} className='w-full text-center'>User</option>
        <option value={'admin'} className='w-full text-center'>Admin</option>
      ...
      <div className="mt-8 flex flex-col-reverse items-center justify-end gap-3 *:w-full sm:flex-row sm:*:w-auto">
        ...
        <ButtonPrimary onClick={createUser} disabled={!(newUser.user_id && newUser.name && newUser.initials)}>Add User</ButtonPrimary>
      </div>
    </ModalContainer>
  )
})
...
function UserRow ({user, apiUpdate}) {
  ...
  return (
    <tr className='hover:bg-zinc-50 hover:shadow-md dark:hover:bg-zinc-800 border-l-8 border-zinc-500'>
      ...
      <TD className='w-36 h-10'> 
        <div onClick={deleteUser} className='cursor-pointer w-full h-full flex items-center justify-center'>
          Delete       
        </div>
      </TD>
  ...
  return (
    <div className='relative mx-auto lg:max-w-7xl '>
      <main className='pb-40'>
        <GridContaier >
          {/* ------- Header ----- */}
          <div className='relative xl:col-span-13 sm:col-span-1 flex items-center pt-4'><H1>Manage Users</H1></div>
          ...
          <Card className='xl:col-span-15 sm:col-span-2'>
            ...
            <div>
              <Table>
                ...
                <tbody>
                  {(dataItems || [])
                    .map((user,i) => <UserRow key={i} user={user} apiUpdate={apiUpdate} />)}
